...
\texttt{initialize} request fetches the server's Python source file from
the GitHub repository and parses all tool definitions; subsequent
\texttt{tools/call} requests are dispatched to an embedded TypeScript
...
\texttt{new~Function()}, which Pyodide requires internally. The
interpreter handles the patterns used by the dominant class of published
MCP servers: \texttt{requests}-based outbound HTTP calls mapped to
native fetch, JSON processing, common string and collection operations,
and standard control flow constructs. Servers relying on async
runtimes, C-extension packages, or non-\texttt{requests} HTTP libraries
fall outside this execution scope.
...
target Python source file from the GitHub raw content URL stored in the
R2 registry record, parses all tool definitions using a structured
source walk, and caches the parsed environment for the session duration.
Tool calls are dispatched to the embedded TypeScript interpreter, which
evaluates the tool function body against the provided arguments and
returns the result as a JSON-RPC response. A 30-minute inactivity alarm
is scheduled on every request and reset on each subsequent call; on
expiry, the Durable Object is evicted and session state is released. The
Worker is deployed and managed via \texttt{wrangler}; live execution
logs are accessible through \texttt{wrangler tail}.
...
repository. The tool discovery stage extracted tool definitions from all
repositories, covering both decorator-based (\texttt{@mcp.tool()}) and
registration-based (\texttt{Tool(name=...)}) definition patterns.
...
runs to ensure cold object instantiation. Because the TypeScript
interpreter requires no dependency installation, cold start latency is
dominated solely by the outbound HTTP round trip to fetch the Python
source file from the GitHub raw content service.
...
network round-trip time to the GitHub raw content service, not by
dependency installation, as the TypeScript interpreter requires no
package installation step. Subsequent requests within the same Durable
...
scope of these results. Execution support is restricted to Python servers and, within that
scope, further constrained to the language features handled by the
embedded TypeScript interpreter; servers relying on async runtimes,
C-extension packages, or HTTP libraries other than \texttt{requests}
require an alternative execution strategy. Node.js and other runtimes
are not yet supported, excluding a significant portion of the MCP
ecosystem. The tool discovery stage relies on regex pattern matching,
...
Future work will target four directions: extending the TypeScript
interpreter and evaluating Cloudflare Workers for Platforms as an
execution substrate for servers requiring full Python runtime support,
thereby broadening coverage to dependency-heavy Python and Node.js
servers; automated repository webhook re-scanning to maintain current
