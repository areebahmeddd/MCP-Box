...
def _device_login(cfg: Config, provider: str) -> None:
    ...
    api_url = cfg.SUPERBOX_API_URL
    if not api_url:
        raise RuntimeError("SUPERBOX_API_URL is required for OAuth device login.")
...
@click.group()
def auth() -> None:
    """Authenticate against the SuperBox registry"""
...
def login(provider: str, email: Optional[str], password: Optional[str]) -> None:
    ...
    try:
        ...
        if _session_active(cfg):
            click.echo("Already logged in. Use 'superbox auth logout' to switch accounts.")
            return
