...
def _check_auth(cfg: Config) -> None:
    ...
    if not tokens or not tokens.get("id_token"):
        ...
        click.echo("Please login first using: superbox auth login")
        sys.exit(1)
    ...
    try:
        ...
        if response.status_code != 200 or not response.json().get("users"):
            ...
            click.echo("Please login again using: superbox auth login")
            sys.exit(1)
    except Exception:
...
@click.command()
@click.option("--name", help="MCP server name (reads from superbox.json if not provided)")
...
def push(
    ...
    try:
        ...
        if not name:
            click.echo("Error: --name required (or create superbox.json with 'superbox init')")
            sys.exit(1)
        ...
        if not repo_url:
            click.echo("Error: repository URL not found in superbox.json under 'repository.url'")
            sys.exit(1)
